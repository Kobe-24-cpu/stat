...
interface MediaItem {
  id: string; type: "video" | "photo"; src: string; title: string;
}
...
export default function Home() {
  ...
  useEffect(() => {
    // ✅ Lecture depuis Supabase (pas localStorage)
  ...
  // Note : dans Supabase la colonne s'appelle "joueur_id" (pas "joueurId")
  function avg(joueurId: string, key: string): string {
    const s = stats.filter((m: any) => m.joueur_id === joueurId);
    if (!s.length) return "0.0";
    return (s.reduce((sum: number, m: any) => sum + (m[key] || 0), 0) / s.length).toFixed(1);
  }
    ...
    <div style={{ width: "100%", overflowX: "hidden", background: "#1C1917", minHeight: "100vh", fontFamily: "'DM Sans', sans-serif", color: "#F5F5F4" }}>
      ...
      <section style={{ maxWidth: 1200, margin: "0 auto", padding: "40px 24px 60px" }}>
        ...
        <div className="media-gallery-container" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))", gap: 16 }}>
          {videos.map((vid, idx) => (
            <div key={vid.id} onClick={() => setActiveMedia(vid)} className="media-card"
              style={{ position: "relative", height: 240, background: "#2C2925", borderRadius: 14, overflow: "hidden", cursor: "pointer", border: "1px solid rgba(249,115,22,0.1)", flexShrink: 0 }}>
              <video src={vid.src} muted playsInline loop autoPlay style={{ width: "100%", height: "100%", objectFit: "cover", opacity: 0.75 }} />
              <div style={{ position: "absolute", inset: 0, background: "linear-gradient(to top, rgba(0,0,0,0.8), transparent)", display: "flex", flexDirection: "column", justifyContent: "flex-end", padding: 14 }}>
          ...
          {photos.map((pic, idx) => (
            <div key={pic.id} onClick={() => setActiveMedia(pic)} className="media-card"
              style={{ position: "relative", height: 240, background: "#2C2925", borderRadius: 14, overflow: "hidden", cursor: "pointer", border: "1px solid rgba(255,255,255,0.05)", flexShrink: 0 }}>
      ...
      {activeMedia && (
        <div onClick={() => setActiveMedia(null)} style={{ position: "fixed", inset: 0, backgroundColor: "rgba(12,10,9,0.95)", zIndex: 9999, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 24, backdropFilter: "blur(8px)" }}>
          <button onClick={() => setActiveMedia(null)} style={{ position: "absolute", top: 24, right: 24, background: "rgba(255,255,255,0.1)", border: "none", color: "white", fontSize: 24, width: 50, height: 50, borderRadius: "50%", cursor: "pointer" }}>✕</button>
          <div onClick={(e) => e.stopPropagation()} style={{ maxWidth: "100%", maxHeight: "70vh", borderRadius: 12, overflow: "hidden", backgroundColor: "#1C1917" }}>
          ...
          <div style={{ marginTop: 20, textAlign: "center" }}>
            ...
            <p style={{ color: "#78716C", fontSize: 14, margin: 0 }}>Cliquez en dehors pour quitter</p>
          </div>
        </div>
      )}

      {/* ── EFFECTIF (depuis Supabase) ── */}
      <section style={{ maxWidth: 1200, margin: "0 auto", padding: "0 24px 80px" }}>
        ...
        {joueurs.length === 0 ? (
          <div style={{ background: "#2C2925", border: "1px dashed rgba(255,255,255,0.1)", borderRadius: 16, padding: 60, textAlign: "center" }}>
            ...
            <p style={{ color: "#78716C", fontSize: 16 }}>Aucun joueur validé pour le moment.</p>
            ...
            {joueurs.map((j: any) => (
              <div key={j.id} style={{ background: "#2C2925", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 14, padding: "20px 24px", display: "flex", alignItems: "center", gap: 16 }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  {j.surnom && <div style={{ fontSize: 12, color: "#F97316" }}>"{j.surnom}"</div>}
                ...
                <div style={{ display: "flex", gap: 12, textAlign: "center" }}>
                  {[
                    { val: avg(j.id, "points"),  label: "pts" },
                    { val: avg(j.id, "rebonds"),  label: "reb" },
                    { val: avg(j.id, "passes"),   label: "pds" },
                  ].map((s) => (
                    <div key={s.label}>
      ...
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        @media (max-width: 768px) {
          ...
          .media-card { width: 270px !important; scroll-snap-align: start; }
          .media-gallery-container::-webkit-scrollbar { height: 4px; }
          .media-gallery-container::-webkit-scrollbar-thumb { background-color: rgba(249,115,22,0.4); border-radius: 10px; }
        }
      `}} />
    </div>
  );
